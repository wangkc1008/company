...
/***/ (function(module, exports, __webpack_require__) {
	...
	exports.aomLocationList = aomLocationList;
	...
	function aomLocationList() {
	  ...
	  // ++ 2017-9-27 20:53:57
	  $("#locationListType").multiselect();
	  // ++ 2017-9-27 21:52:51 所属区域修改
	  var tree = new _pccTree.PCCTree('#crossingTree', {
	    ajaxLoadUrl: ajaxURL.PCCUrl
	  });
	  var treeModal = new _pccTree.PCCTree('#crossingTreeModal', {
	    ajaxLoadUrl: ajaxURL.PCCUrl
	  });
	  // 关联卡口、关联设备点击切换显示与隐藏
	  $('.switch-group').on('click', '.equip-switch', function (e) {
	    if ($(this).hasClass('open')) {
	      $(this).removeClass('open');
	      $(this).siblings('.switch-box').hide();
	    } else {
	      $(this).addClass('open');
	  ...
	  $('.js-edit').on('click', function () {
	    ...
	    temp.id = tr.data('id');
	    temp.key_id = tr.data('key');
	    temp.feature = tr.data('feature');
	    temp.text = tr.data('text');
	    temp.lng = tr.data('lng');
	    temp.lat = tr.data('lat');
	    temp.wifiId = tr.data('wifiid') + '';
	    temp.wifiText = tr.data('wifitext');
	    temp.point = tr.data("point") + '';
	    temp.locationType = tr.data("locationtype");
	    temp.typeId = tr.data("typeid") + '';
	  ...
	  $('#addLocation').on('click', function () {
	    ...
	    $('#rest').val('add');
	    //该卡口3公里内的点
	    equipAjaxLngLat(mapConfig.center[0], mapConfig.center[1]);
	    ajaxLngLat(mapConfig.center[0], mapConfig.center[1]);
	  });

	  $('#save').on('click', function () {
	    // if ($("#associatePoint").val()) {
	    form.find('input[name=associate_point]').val($("#associatePoint").val());
	    // }
	    // if ($("#associateEquip").val()) {
	    form.find('input[name=associate_equip]').val($("#associateEquip").val());
	    // }

	    var url = '';
	    var lid = form.find("input[name=location_id]").val(); // 点位编号
	    var lname = form.find('input[name=location_name]').val(); // 点位名称
	    var region = form.find("input[name=region_id]").val(); // 所属区域
	    var lng = form.find('input[name=lng]').val(); // 经度
	    var lat = form.find('input[name=lat]').val(); // 纬度
	    var point = form.find('input[name=associate_point]').val(); // 关联卡口
	    var equip = form.find('input[name=associate_equip]').val(); // 关联设备
	  ...
	  $('#editModal').on('shown.bs.modal', function () {
	    ...
	    mapCtrl._markerAll = [];
	    if (temp.id) {
	      $("input[name=location_id]").val(temp.id); // 请输入点位编号
	      $("input[name=old_id]").val(temp.id);
	      $("input[name=key_id]").val(temp.key_id);
	      $('input[name=location_name]').val(temp.text); // 请输入点位名称
	      form.find("input[name=region_id]").val(temp.regionId);
	      $('input[name=lng]').val(temp.lng);
	      $('input[name=lat]').val(temp.lat);
	      $('input[name=associate_point]').val(temp.point);
	      $('input[name=associate_equip]').val(temp.wifiId);
	      if (temp.typeId) {
	        $("#locationListType").find('option[value=' + temp.typeId + ']').attr("selected", true);
	        $("#locationListType").multiselect("rebuild");
	      }
	      // 所属区域
	      if (!temp.regionId) {
	        var treeNode = treeModal.treeObj.getCheckedNodes(true);
	        if (treeNode.length) {
	          treeModal.treeObj.checkNode(treeNode[0], false, true, false);
	        }
	        form.find("#regionName").val("");
	      } else {
	        try {
	          treeModal.check(temp.regionId);
	          form.find("#regionName").val(temp.regionName);
	        } catch (e) {
	          var _treeNode = treeModal.treeObj.getCheckedNodes(true);
	          if (_treeNode.length) {
	            treeModal.treeObj.checkNode(_treeNode[0], false, true, false);
	          }
	          form.find("#regionName").val("");
	        }
	      }
	      ...
	      //该卡口3公里内的点
	      var equip = temp.wifiId;
	      equipAjaxLngLat(temp.lng, temp.lat, equip);
	      //该卡口3公里内的点
	      var point = temp.point;
	      ajaxLngLat(temp.lng, temp.lat, point);
	    } else {
	      marker = mapCtrl.addMarkerOne(mapConfig.center[0], mapConfig.center[1], '', '', 'stop-marker', 1);
	      $("input[name=location_id]").val(''); // 请输入点位编号
	      $('input[name=location_name]').val(''); // 请输入点位名称
	      form.find("input[name=region_id]").val('');
	      form.find("#regionName").val('');
	      $('input[name=lng]').val('');
	      $('input[name=lat]').val('');
	      $('input[name=associate_point]').val('');
	      $('input[name=associate_equip]').val('');
	    }
	    marker.addEventListener(IMAP.Constants.DRAG_END, function (e) {
	      var lng = e.lnglat.lng,
	          lat = e.lnglat.lat;
	      $('#editLocationForm').find('input[name="lng"]').val(lng);
	      $('#editLocationForm').find('input[name="lat"]').val(lat);
	      //该卡口3公里内的点
	      equipAjaxLngLat(lng, lat);
	      ajaxLngLat(lng, lat);
	    }, marker);
	  });

	  function equipAjaxLngLat(lng, lat, point) {
	    (0, _ajaxLoadParse.ajaxLoad)({
	      url: ajaxURL.wifiLocation,
	      data: { "lng": lng, "lat": lat },
	      dataType: "json"
	    }, function (res) {
	      var _res$status = res.status,
	          status = _res$status === undefined ? 1 : _res$status,
	          data = res.data,
	          html = "";
	      ...
	      if (!parseInt(status)) {
	        data.forEach(function (item) {
	          ...
	          mapCtrl.addMarkerOne(item.lng, item.lat, item.id, '', 'phone', '', item.text);
	        });

	        $("#associateEquip").html(html);
	        $("#associateEquip").multiselect("rebuild");

	        if (point && point.length > 0) {
	          (function () {
	            var pointArr = point.split(",");
	            $("#associateEquip").multiselect('select', pointArr);
	            $("#associateEquip").multiselect("rebuild");

	            var _loop = function _loop(i) {
	              var marker = mapCtrl._markerAll.find(function (item) {
	                return item.id == pointArr[i];
	              });
	              if (marker) {
	                mapCtrl.iconPicked(marker);
	                checkedMarkerIdArr.push(marker);
	              }
	            };
	        ...
	        // for (let i = 0; i < mapCtrl._markerAll.length; i++) {
	        //   if (mapCtrl._markerAll[i].id) {
	        //     mapCtrl.markerOneClick(mapCtrl._markerAll[i]);
	        //   }
	        // }
	        pointListChk();
	        // multiSelect();
	        if ($(".multiselect-container").height() > 300) {
	          $(".multiselect-container").css({ "maxHeight": "300px", "overflow-y": "scroll" });
	        }
	      }
	    });
	  }

	  function ajaxLngLat(lng, lat, point) {
	    (0, _ajaxLoadParse.ajaxLoad)({
	      url: ajaxURL.kkLocation,
	      data: { "lng": lng, "lat": lat },
	      dataType: "json"
	    }, function (res) {
	      var _res$status2 = res.status,
	          status = _res$status2 === undefined ? 1 : _res$status2,
	          data = res.data,
	          html = "";
	      ...
	      if (!parseInt(status)) {
	        data.forEach(function (item) {
	          ...
	          mapCtrl.addMarkerOne(item.lng, item.lat, item.id, '', '1', '', item.text);
	        });
	        $("#associatePoint").html(html);
	        $("#associatePoint").multiselect("rebuild");
	        if (point && point.length > 0) {
	          (function () {
	            var pointArr = point.split(",");
	            $("#associatePoint").multiselect('select', pointArr);
	            $("#associatePoint").multiselect("rebuild");
	            ...
	            var _loop2 = function _loop2(i) {
	              ...
	              if (marker) {
	                mapCtrl.iconPicked(marker);
	                checkedMarkerIdArr.push(marker.id);
	              }
	            };
	        ...
	        for (var i = 0; i < mapCtrl._markerAll.length; i++) {
	          if (mapCtrl._markerAll[i].id) {
	            mapCtrl.markerOneClick(mapCtrl._markerAll[i], checkedMarkerIdArr);
	          }
	        }
	        pointListChk();
	        // multiSelect();
	  ...
	  function pointListChk() {
	    ...
	    $(".multiselect-container input[type='checkbox']").on("click", function () {
	      ...
	      if (_this.prop("checked")) {
	        var id = _this.val();
	        if (id && id != "undefined") {
	          var _marker = mapCtrl._markerAll.find(function (item) {
	            return item.id == id;
	          });
	          //修改icon
	          if (_marker && _marker != "undefined")
	            // mapCtrl.iconRecover(marker);
	            mapCtrl.iconPicked(_marker);
	        }
	      } else {
	        var _id = _this.val();
	        if (_id && _id != "undefined") {
	          var _marker2 = mapCtrl._markerAll.find(function (item) {
	            return item.id == _id;
	          });
	          if (_marker2 && _marker2 != "undefined") mapCtrl.iconRecover(_marker2);
	        }
	        //mapCtrl.iconPicked(marker);
	      }
	    });
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _yisaSelect = __webpack_require__(8);
